...
exports.INITIAL_MODELS = exports.PROVIDERS = void 0;
...
const ModelInfoTooltip = ({ model, provider }) => (<div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-72 p-5 bg-slate-900 border border-slate-700 rounded-3xl shadow-2xl z-[200] opacity-0 group-hover/info:opacity-100 pointer-events-none transition-all duration-300 transform translate-y-2 group-hover/info:translate-y-0 backdrop-blur-2xl ring-1 ring-white/10">
    ...
    {model.tags && (<div className="flex flex-wrap gap-1.5 mb-4">
        {model.tags.map(t => (<span key={t} className="text-[8px] font-bold text-slate-500 bg-slate-800/50 px-2 py-0.5 rounded border border-slate-700/50 uppercase">#{t}</span>))}
      </div>)}
...
const InputPanel = ({ query, setQuery, handleSubmit, handleCancel, isLoading, useAutomation, setUseAutomation, selectedModels, onModelChange, requestCounts, onShowCancel, answerMode, setAnswerMode }) => {
    const [activeTab, setActiveTab] = (0, react_1.useState)('mission');
    const [registry, setRegistry] = (0, react_1.useState)(exports.INITIAL_MODELS);
    const [searchQuery, setSearchQuery] = (0, react_1.useState)('');
    const [activeCategory, setActiveCategory] = (0, react_1.useState)('All');
    const [showMegaMenu, setShowMegaMenu] = (0, react_1.useState)(false);
    const [authModal, setAuthModal] = (0, react_1.useState)({ open: false });
    const [customEndpointModal, setCustomEndpointModal] = (0, react_1.useState)({ open: false });
    const [keyInput, setKeyInput] = (0, react_1.useState)('');
    const [customUrl, setCustomUrl] = (0, react_1.useState)('');
    const [customModelId, setCustomModelId] = (0, react_1.useState)('');
    const fileInputRef = (0, react_1.useRef)(null);
    (0, react_1.useEffect)(() => {
        const checkLocalStatus = async () => {
            const updatedRegistry = await Promise.all(registry.map(async (m) => {
                const provider = exports.PROVIDERS.find(p => p.id === m.providerId);
                if (provider?.isLocal) {
                    try {
                        ...
                        return { ...m, connectionStatus: 'connected' };
                    }
                    catch (err) {
                        return { ...m, connectionStatus: 'error' };
                    }
                }
                return m;
            }));
    ...
    const updateModelStatus = (id, status, key) => {
        setRegistry(prev => prev.map(m => m.id === id ? { ...m, connectionStatus: status, apiKey: key || m.apiKey } : m));
    };
    const handleConnect = async (model) => {
        const provider = exports.PROVIDERS.find(p => p.id === model.providerId);
        if (!provider)
            return;
        if (provider.isLocal) {
        ...
        if (provider.id === 'google' && window.aistudio?.openSelectKey) {
    ...
    return (<div className="flex flex-col gap-6 sticky top-8">
      {/* Navigation */}
          ...
          <input type="file" ref={fileInputRef} onChange={(e) => {
            ...
            reader.onload = (event) => {
                try {
                    ...
                    alert("Invalid Vault JSON.");
                }
            };
            reader.readAsText(file);
        }} className="hidden" accept=".json"/>
          <button onClick={() => fileInputRef.current?.click()} className="p-3 rounded-xl bg-slate-900 border border-slate-800 text-emerald-500 hover:border-emerald-500/50 transition-all shadow-md" title="Link Vault"><icons_1.ShieldIcon className="w-5 h-5"/></button>
          <button onClick={() => setShowMegaMenu(!showMegaMenu)} className="p-3 rounded-xl bg-slate-900 border border-slate-800 text-violet-500 hover:border-violet-500/50 transition-all shadow-md" title="Add Endpoint"><icons_1.BrainCircuitIcon className="w-5 h-5"/></button>
        </div>
      </div>

      {showMegaMenu && (<div className="absolute top-20 right-0 w-80 bg-slate-900 border border-slate-700/50 rounded-[2rem] p-6 shadow-2xl z-[100] animate-fade-in backdrop-blur-2xl ring-1 ring-white/10">
          <div className="flex justify-between items-center mb-6 px-1">
            <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Connect Instance</h3>
            <button onClick={() => setShowMegaMenu(false)} className="text-slate-600 hover:text-white"><icons_1.CloseIcon className="w-4 h-4"/></button>
          </div>
          <div className="grid grid-cols-2 gap-2">
            {exports.PROVIDERS.map(p => (<button key={p.id} onClick={() => { setCustomUrl(p.baseUrl || ''); setCustomEndpointModal({ open: true, provider: p }); setShowMegaMenu(false); }} className="flex items-center gap-2.5 p-3 bg-slate-950/50 border border-slate-800 rounded-xl hover:bg-violet-500/10 hover:border-violet-500/30 transition-all text-left group">
      ...
      {activeTab === 'mission' ? (<div className="bg-slate-900/90 border border-slate-800 rounded-[2.5rem] p-6 sm:p-8 shadow-2xl animate-fade-in backdrop-blur-xl space-y-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <icons_1.PaperAirplaneIcon className="w-5 h-5 text-violet-500 rotate-45"/>
            ...
            <div className="flex bg-slate-950/60 p-1 rounded-xl border border-slate-800 shadow-inner">
              <button onClick={() => setAnswerMode('simple')} className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${answerMode === 'simple' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-600 hover:text-slate-400'}`}>
                Simple
              </button>
              <button onClick={() => setAnswerMode('complex')} className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${answerMode === 'complex' ? 'bg-violet-600/20 text-violet-400 border border-violet-500/20 shadow-md' : 'text-slate-600 hover:text-slate-400'}`}>
                Complex
              </button>
            </div>
          </div>
          
          <textarea value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Input objective for parallel deliberation..." className="w-full h-44 bg-slate-950/80 border border-slate-800 rounded-3xl p-6 text-sm text-slate-100 focus:ring-2 focus:ring-violet-500/50 outline-none resize-none transition-all shadow-inner placeholder:text-slate-800" disabled={isLoading}/>
          <div className="space-y-6">
            <div className="grid grid-cols-1 gap-4">
              {[types_1.AgentRole.Model1, types_1.AgentRole.Model2, types_1.AgentRole.Model3].map(role => (<div key={role} className="space-y-2">
                   <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">{role} Specialist</label>
                   <select value={selectedModels[role]} onChange={(e) => onModelChange(role, e.target.value)} className="w-full bg-slate-950/50 border border-slate-800 rounded-xl p-3 text-[11px] text-slate-200 outline-none focus:border-violet-500/50 appearance-none transition-all">
                     {registry.filter(m => m.connectionStatus === 'connected' || m.isSystemModel).map(m => (<option key={m.id} value={m.id} className="bg-slate-900">{m.label} ({m.providerId})</option>))}
                   </select>
                </div>))}
            </div>
        ...
        </div>) : (<div className="bg-slate-900/90 border border-slate-800 rounded-[2.5rem] p-6 sm:p-8 shadow-2xl animate-fade-in backdrop-blur-xl flex flex-col gap-6 h-[80vh] min-h-[500px]">
          ...
          <div className="flex flex-col gap-3 overflow-y-auto pr-1 custom-scrollbar flex-grow">
            {filteredModels.map(m => {
                ...
                return (<div key={m.id} className={`p-4 rounded-2xl border transition-all relative group flex flex-col gap-3 ${m.connectionStatus === 'connected' ? 'bg-emerald-500/5 border-emerald-500/20 shadow-[inset_0_0_20px_rgba(16,185,129,0.02)]' : 'bg-slate-950/60 border-slate-800 hover:border-violet-500/30'}`}>
                  <div className="flex justify-between items-start gap-3">
                    ...
                    <div className="relative group/info flex-shrink-0">
                      <div className="p-1.5 hover:bg-slate-800 rounded-lg transition-colors cursor-help"><icons_1.InfoIcon className="w-4 h-4 text-slate-600 group-hover/info:text-violet-400"/></div>
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-slate-800/40">
                    ...
                    {m.connectionStatus !== 'connected' && (<button onClick={(e) => { e.stopPropagation(); handleConnect(m); }} className="text-[9px] font-black uppercase text-violet-400 hover:text-white transition-all px-3 py-1 bg-violet-600/10 rounded-lg border border-violet-500/20 active:scale-95">Establish Link</button>)}
                  </div>
                </div>);
            })}
          </div>
      ...
      {(customEndpointModal.open || authModal.open) && (<div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[300] w-[90%] max-w-sm pointer-events-none flex items-center justify-center animate-fade-in">
          <div className="bg-slate-900 border border-slate-700/50 rounded-[2rem] p-6 sm:p-8 w-full shadow-[0_0_100px_rgba(0,0,0,0.8)] relative ring-1 ring-white/10 pointer-events-auto">
            <button onClick={() => { setCustomEndpointModal({ open: false }); setAuthModal({ open: false }); }} className="absolute top-6 right-6 text-slate-500 hover:text-white"><icons_1.CloseIcon className="w-4 h-4"/></button>
            ...
            {customEndpointModal.open && (<>
                ...
                <div className="space-y-4">
                  {customEndpointModal.provider?.isLocal && (<div className="bg-amber-500/5 border border-amber-500/10 rounded-lg p-3">
                  ...
                  {!customEndpointModal.isExisting && (<div className="space-y-1.5">
            ...
            {authModal.open && (<>
                ...
                <div className="space-y-4">
                  ...
                  <button onClick={() => { if (authModal.modelId && keyInput) {
                updateModelStatus(authModal.modelId, 'connected', keyInput);
                setKeyInput('');
                setAuthModal({ open: false });
            } }} className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-lg uppercase tracking-[0.2em] text-[10px] shadow-lg active:scale-95 transition-all">Establish Link</button>
                </div>
              </>)}
          </div>
        </div>)}
...
//# sourceMappingURL=TicketInputForm.js.map