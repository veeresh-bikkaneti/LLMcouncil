...
const ConsensusDashboard = ({ consensus, chairpersonStatus, chairpersonUsage, originalQuery, agentAnalyses }) => {
    ...
    const renderContent = () => {
        if (chairpersonStatus === 'idle') {
            return (<div className="flex flex-col items-center justify-center py-24 text-slate-600 border border-dashed border-slate-800 rounded-[2rem] bg-slate-900/10">
        ...
        if (chairpersonStatus === 'thinking') {
            return (<div className="flex flex-col items-center justify-center py-24 text-slate-400 bg-slate-900/20 rounded-[2rem] border border-violet-500/20">
        ...
        if (chairpersonStatus === 'error') {
            return (<div className="bg-red-950/30 p-12 rounded-[2rem] border border-red-800/40 text-red-300 text-center">
        ...
        if (consensus) {
            return (<div className="bg-slate-950/70 p-10 rounded-[2.5rem] border border-slate-800/80 shadow-[0_0_50px_rgba(0,0,0,0.3)] animate-fade-in relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-8 opacity-[0.03] group-hover:opacity-10 transition-all duration-700">
                <icons_1.ChairpersonIcon className="w-56 h-56"/>
            </div>
            <div className="relative z-10">
                <Markdown_1.Markdown text={consensus.comprehensiveAnswer}/>
                <div className="mt-12 flex justify-end">
                  <button onClick={handleDownload} className="flex items-center gap-2 px-6 py-3 bg-violet-600/10 hover:bg-violet-600/20 text-violet-400 border border-violet-600/20 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all hover:scale-105 active:scale-95 shadow-xl">
                    <icons_1.DownloadIcon className="w-4 h-4"/> Export Report
                  </button>
                </div>
            </div>
        </div>);
    ...
    return (<div className="bg-slate-900/95 border border-slate-800 rounded-[2.5rem] shadow-2xl p-10 backdrop-blur-xl">
      <div className="flex items-center justify-between mb-12">
        <div className="flex items-center">
            <div className="p-4 bg-violet-500/10 rounded-2xl mr-6 border border-violet-500/20 shadow-inner">
                <icons_1.ChairpersonIcon className="w-9 h-9 text-violet-400"/>
            </div>
            <div>
                ...
                <div className="flex items-center gap-5 mt-1.5">
                  ...
                  {chairpersonUsage && (<div className="flex items-center gap-2.5 border-l border-slate-800 pl-4">
...
exports.default = ConsensusDashboard;
//# sourceMappingURL=ConsensusDashboard.js.map