...
const Markdown = ({ text }) => {
    ...
    return (<div className="text-slate-300 text-sm leading-relaxed space-y-4 font-medium">
      {parts.map((part, i) => {
            if (part.startsWith('```')) {
                const match = part.match(/```(\w+)?\n?([\s\S]*?)```/);
                const lang = match?.[1] || '';
                const code = match?.[2] || '';
                return (<pre key={i} className="bg-slate-950 p-5 rounded-2xl border border-slate-800 overflow-x-auto my-6 font-mono text-[11px] text-violet-300 shadow-inner">
            ...
            const flushTable = () => {
                if (inTable && tableRows.length > 0) {
                    ...
                    renderedElements.push(<div key={`table-${renderedElements.length}`} className="overflow-x-auto my-8 rounded-2xl border border-slate-800 shadow-2xl bg-slate-900/20">
                <table className="w-full border-collapse text-[12px]">
                  <thead className="bg-slate-900/80 border-b border-slate-800">
                    <tr>
                      {headerRow.map((cell, idx) => (<th key={idx} className="p-4 text-left font-black uppercase tracking-[0.1em] text-violet-400 whitespace-nowrap">{cell.trim()}</th>))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800/50">
                    {dataRows.map((row, rIdx) => (<tr key={rIdx} className="hover:bg-violet-500/[0.03] transition-colors">
                        {row.map((cell, cIdx) => (<td key={cIdx} className="p-4 text-slate-400 font-medium">{parseInline(cell.trim())}</td>))}
                      </tr>))}
                  </tbody>
                </table>
              </div>);
            ...
            lines.forEach((line, lIdx) => {
                ...
                if (trimmed.startsWith('#')) {
                    ...
                    renderedElements.push(<h4 key={lIdx} className={`${sizes[level] || 'text-base'} font-black text-white uppercase tracking-tight mt-10 mb-5 border-b border-slate-800/50 pb-3 flex items-center gap-3`}>
