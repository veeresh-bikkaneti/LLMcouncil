...
mode (top-right toggle) to run a small instruct model (default: Qwen2.5 0.5B, quantized)
entirely client-side via [`@mlc-ai/web-llm`](https://github.com/mlc-ai/web-llm) and WebGPU
— no server, no account, no key.

Because a ~0.5B model is too small to reliably call tools on its own, Local Assistant mode
never relies on LLM-native function calling. Instead a plain JavaScript orchestrator
(`src/engine/chatbot.ts`) runs the retrieval step itself before the model ever sees the
query:
...
(a few hundred MB to ~1.3GB depending on which one you pick) download once and are
