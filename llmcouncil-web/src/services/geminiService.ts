...
import { AgentRole, type AgentAnalysis } from '../types';

const ai = new GoogleGenAI({ apiKey: process.env.API_KEY! });

const getModelConfig = (useThinkingMode: boolean) => {
  if (useThinkingMode) {
    return {
...
export const sanitizeText = async (query: string): Promise<string> => {
...
export const analyzeWithAgent = async (role: AgentRole, query: string, useThinkingMode: boolean): Promise<string> => {
...
export const synthesizeConsensus = async (query: string, analyses: AgentAnalysis[], useThinkingMode: boolean): Promise<string> => {
    const { model, config } = getModelConfig(useThinkingMode);

    const model1Response = analyses.find(a => a.role === AgentRole.Model1)?.analysis || "N/A";
    const model2Response = analyses.find(a => a.role === AgentRole.Model2)?.analysis || "N/A";
    const model3Response = analyses.find(a => a.role === AgentRole.Model3)?.analysis || "N/A";
