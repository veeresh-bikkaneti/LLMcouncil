...
class CouncilOrchestrator {
    ...
    async runCouncil(text, onProgress) {
        const analysisId = `analysis-${Date.now()}`;
        const timestamp = Date.now();
        // Initialize agent outputs
        const agentOutputs = agents_1.AGENTS.map(agent => ({
            agent,
            status: 'idle',
            response: '',
        }));
        // Run agents in parallel
        const analysisPromises = agents_1.AGENTS.map(async (agent, index) => {
            try {
                agentOutputs[index].status = 'thinking';
                onProgress?.(agent.id, 'thinking');
                const response = await this.llmClient.analyzeWithAgent(agent.id, text, (status) => onProgress?.(agent.id, status.toLowerCase()));
        ...
        try {
            ...
            const consensusRate = (successfulAgents / agents_1.AGENTS.length) * 100;
            return {
                id: analysisId,
                timestamp,
                query: text,
